...
- **項目は途中の月から追加できる**。`effectiveBudget()` は最初のエントリより前の月には0を返し、
  `isActiveInMonth()` が「予算も支出も繰越も無い行」を画面から隠すので、追加前の月の実績は変わらない
  (本番では 2026-09 から「引っ越し準備金」20,000円/月 を追加済み)
...
タスク名 **ふたり家計簿バックアップ** / **毎週月曜 13:00 + 毎月末日 13:00**(トリガー2つ) /
実行アカウントはログオン中のユーザー。
