...
- **買い物リスト**(2026-09-13 追加): 下部メニュー「買い物」。品目は `households/{id}/shopping/{itemId}`
  に1品1文書で保存(二人が店で同時にチェックしても互いを上書きしない)。項目ごとにまとめて残り予算を表示し、
  チェックした品目は「✓の○品を〇〇に記録する」で項目・メモを引き継いで支出入力へ。**支出の保存に成功してから**
  品目をリストから消す。`getData()` には含めないので、バックアップの書き出し・復元の対象外。
  読み込み失敗は買い物リスト画面の中だけで知らせ、家計簿全体をエラー画面にしない。
  月次レポートは下部メニューから外し、履歴画面の上部ボタンから開く
- **入力途中の画面を守る**: ストアの変更(相方の操作)が届いても、入力・編集・設定画面で入力途中なら
  画面を作り直さない(`formDirty`。バッジだけ更新)。買い物リスト画面は入力欄の値・フォーカス・カーソル位置を保持する
...
画面の動作確認は localhost で行う。localhost でだけ `window.__coupleBudget = { store, render }` が
使えるので、コンソールから `__coupleBudget.store.addExpense(...)` などを呼ぶと「相方の操作」を再現できる。
